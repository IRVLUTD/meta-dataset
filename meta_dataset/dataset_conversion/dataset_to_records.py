...
tf.flags.DEFINE_string(
    'tesla_data_root',
    '',
    'Path to the root of the TESLA data.')
...
def write_example(data_bytes,
    ...
    Create a protocol buffer with an integer feature for the class label, bytes feature for set info
    and a bytes feature for the image.
...
def get_example(data_bytes,
    ...
    # will indicate  whether the image belongs to
    # "support" / "query" / "both" indicated by b''
    """Create and return an Example protocol buffer for the given image.

    Returns a protocol buffer with an integer feature for the class label, bytes feature for set info
    and a bytes feature for the image.
...
def gen_rand_split_inds(num_train_classes, num_valid_classes, num_test_classes):
    ...
    train_inds = np.random.choice(
        trainval_inds, num_train_classes, replace=False)
...
def write_tfrecord_from_image_files(class_files,
    ...
    for i, path in enumerate(class_files):
        ...
        try:
            ...
            if skip_on_error:
                logging.warn(
                    'While trying to load file %s, got error: %s', path, e)
            else:
                raise
        else:
...
def write_tfrecord_from_image_files_with_set_info(class_files,
    ...
    Args:
      ...
      k_support: num of support examples (shots)
      k_query: num of query examples (shots)
      invert_img: change black pixels to white ones and vice versa. Used for
        Omniglot for example to change the black-background-white-digit images
        into more conventional-looking white-background-black-digit ones.
      bboxes: list of bounding boxes, one for each filename passed as input. If
    ...
    real_images_count = 0
    example_strings = []
    dataset_is_tesla = dataset_name == "tesla"
    ...
    for i, path_with_set_info in enumerate(class_files):
        ...
        try:
            ...
            include_image = not (
                width < inclusion_threshold or height < inclusion_threshold)
...
def write_tfrecord_from_tesla_directory_structure(class_directory,
    ...
    Returns:
      The number of images (without replication which is done for tesla dataset) 
      written into the records file.
    """
    ...
    for class_set in ["support", "query"]:
        class_files.extend(get_classes_with_set_info(
            class_directory, class_set))
...
class DatasetConverter(object):
    ...
    def _init_specification(self):
        ...
        self.images_per_class = collections.defaultdict(
            dict if self.name == 'tesla' else int)
...
class TeslaConverter(DatasetConverter):
    ...
    def parse_split_data(self, split, classes, train_or_test_path):
        ...
        for class_name in tqdm(classes):
            ...
            real_images_count, output_path = write_tfrecord_from_tesla_directory_structure(
                class_path, class_label, class_records_path, self.name,
    ...
    """
        ...
        # for train experiments use following validation_classes
        validation_classes = [
            "product_box",
            "eagle",
            "wood_tower",
        ...
        # for real world experiments use following validation_classes
        # validation_classes = [
        #     "REAL_WOOD_BOX",
        #     "REAL_FLASH_DRIVE",
        #     "REAL_HAMMER",
        ...
        assert (len(training_classes) + len(validation_classes)) \
            in [125, 323]  # for train and real world(4.3) experiments

        data_path_test = os.path.join(self.data_root, 'test_data')
        test_classes = sorted(tf.io.gfile.listdir(data_path_test))
        assert len(test_classes) \
            in [11, 13, 41, 52, 198]  # for variants and real world experiments
    ...
    def create_dataset_specification_and_records(self):
        ...
        for class_id, class_name in enumerate(all_classes):
            ...
            class_directory = os.path.join(
                self.data_root, 'images', class_name)
            class_records_path = os.path.join(
                self.records_path, self.dataset_spec.file_pattern.format(class_id))
...
class MSCOCOConverter(DatasetConverter):
    ...
    def __init__(self,
        ...
        if box_scale_ratio < 1.0:
            raise ValueError(
                'Box scale ratio must be greater or equal to 1.0.')
    ...
    def create_dataset_specification_and_records(self):
        """Implements DatasetConverter.create_dataset_specification_and_records."""
        splits = self.get_splits()
        self.classes_per_split[learning_spec.Split.TRAIN] = len(
            splits['train'])
        self.classes_per_split[learning_spec.Split.VALID] = len(
            splits['valid'])
        ...
        for class_id in range(self.num_all_classes):
            output_path = os.path.join(
                self.records_path, self.dataset_spec.file_pattern.format(class_id))
            class_tf_record_writers.append(
                tf.python_io.TFRecordWriter(output_path))
        ...
        for i, annotation in enumerate(self.coco_instance_annotations):
            try:
                ...
                logging.warning(
                    'Image can not be cropped and will be skipped.')
                continue
...
class ImageNetConverterV2(ImageNetConverter):
    ...
    def _create_data_spec(self):
        super(ImageNetConverterV2, self)._create_data_spec(
