...
def decode_fn(record_bytes):
  return tf.io.parse_single_example(
      ...
      # Schema
      {"image": tf.io.FixedLenFeature([], dtype=tf.int64),
       "label": tf.io.FixedLenFeature([], dtype=tf.float32),
       "set": tf.io.FixedLenFeature([], dtype=tf.string),
       }
  )
...
def decode_example_strings(example_strings):
  for batch in example_strings.map(decode_fn):
    print("image={image} label = {x:.4f},  set = {set}".format(**batch))
    break    
