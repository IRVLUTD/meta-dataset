...
"""Classification of all test set query images"""
...
class TrainerTest():
  ...
  def get_checkpoint(self, EXPNAME, BESTNUM):
      """
      Returns path for the best trained model
      Args:
        EXPNAME: experiment-setup, e.g. baseline-60
        BESTNUM: Best checkpoint update number
      """
      return f'experiment_output/tesla-unseen/checkpoints/${EXPNAME}/model_${BESTNUM}.ckpt'
  ...
  def test_trainer(self):
    ...
    models =  {
        'prototypical': (learners.PrototypicalNetworkLearner, ),
        'matching': (learners.PrototypicalNetworkLearner, ''),
        'maml': (learners.PrototypicalNetworkLearner, ''),
        'maml_init_with_proto': (learners.PrototypicalNetworkLearner, ''),
        'crosstransformer': (learners.PrototypicalNetworkLearner, ''),
        'crosstransformer_simclreps': (learners.PrototypicalNetworkLearner, ''),
    }

    perform_filtration = True
    EXPNAME = prototypical

    trainer_instance = trainer.Trainer(
        train_learner_class=learners.PrototypicalNetworkLearner,
        eval_learner_class=learners.PrototypicalNetworkLearner,
        is_training=False,
        train_dataset_list=['tesla'],
        eval_dataset_list=['tesla'],
        restrict_classes={},
        restrict_num_per_class={},
        checkpoint_dir=self.get_checkpoint(),
        summary_dir='',
        ...
        decay_every=5000,
        decay_rate=0.5,
        experiment_name=f'classify-{EXPNAME}', # needs to be changed
        pretrained_source='scratch', # needs to be changed
        distribute=False,
        enable_tf_optimizations=True,
        normalized_gradient_descent=False,
    )

    # Only a single episode is needed
    providers.Episode(
