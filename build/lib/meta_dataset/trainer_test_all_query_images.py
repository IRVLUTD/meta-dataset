  ...
  def _build_episode(self, split):
    ...
    if len(dataset_spec_list) == 1:
      ...
      with gin.config_scope(gin_scope_name):
        # data_pipeline = pipeline.make_one_source_episode_pipeline_from_tesla_support_query_tfrecords(
        #     tesla_variant="tesla-mixture",
        #     image_size=image_size,
        #     simclr_episode_fraction=simclr_episode_fraction)
        data_pipeline = pipeline.make_one_source_single_episode_pipeline_for_all_tesla_test_classes(
          dataset_spec_list[0],
